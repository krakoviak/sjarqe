NIL_VOWEL     = {:nil => 5}
SHORT_VOWELS  = {:a => 6, :ya => 3, :e => 5, :ye => 2, :u => 6, :i => 3, :o => 7, :yo => 2}
LONG_VOWELS   = {:aa => 0, :ee => 0, :uu => 0, :ii => 0, :oo => 0}
DIPHTONGS     = {:ai => 3, :ei => 3, :yei => 1, :oi => 2, :ou => 2, :au => 2}

VOWELS = NIL_VOWEL.merge(SHORT_VOWELS).merge(LONG_VOWELS).merge(DIPHTONGS)
  ...
  structure[:structure].each do |text_block|
    if text_block == :line
      ...
      structure[text_block].each do |line_type|
        ...
        rhymes[rhyme_key] ||= { :c2 => get_consonant, :v2 => get_vowel, :c1 => get_consonant, :v1 => get_vowel, :c0 => rand(2) > 0 ? get_consonant : nil }
        ...
        while i < vowel_count
          ...
          if (word_start && cons_sequence == 0) || (cons_sequence < MAX_CONSONANT_SEQUENCE && !word_start)
            consonant = (vowel_count - i) < 3 ? cons_rhyme[rhyme["c#{(vowel_count - i)}".to_sym].to_s].split.sample : get_consonant
            #puts "before: #{consonant}"
            line << consonant.to_s
          end

          vowel = vowel_count - i <= 2 ? rhyme["v#{vowel_count - i}".to_sym] : get_vowel
